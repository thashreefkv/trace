...
interface FolderLinkPickerProps {
  folderId: string;
  folderName: string;
  existingLinks: FileLinkRef[];
  onClose: () => void;
  onLinked: () => void;
}
...
export function FolderLinkPicker({ folderId, folderName, existingLinks, onClose, onLinked }: FolderLinkPickerProps) {
  ...
  const linkedSet = useMemo(
    () => new Set(existingLinks.map((l) => `${l.entityKind}:${l.entityId}`)),
    [existingLinks],
  );
  ...
  async function handleLink(entityKind: FileEntityKind, entityId: string) {
    ...
    setError(null);
    try {
      await linkFolderToEntity(folderId, entityKind, entityId);
      onLinked();
    } catch (e) {
      setError(String(e));
    } finally {
      ...
      >
        <div className="flex items-start justify-between border-b border-zinc-100 px-4 py-3">
          <div className="min-w-0">
            <p className="page-kicker">Link folder</p>
            <h3 className="truncate text-[15px] font-semibold text-zinc-950">{folderName}</h3>
          </div>
          <button
            aria-label="Close"
                  ...
                  <li key={key}>
                    <button
                      className={[
                        ...
                        linked ? "bg-emerald-50 text-emerald-900" : "hover:bg-zinc-50 text-zinc-800",
                      ].join(" ")}
                    ...
                    >
                      <span className="min-w-0">
                        ...
                        {r.subtitle ? (
                          <span className="block truncate text-[11px] text-zinc-500">{r.subtitle}</span>
                        ) : null}
                      </span>
